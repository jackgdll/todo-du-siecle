...
<Disclosure let:open>
  <DisclosureButton class="flex w-full items-center gap-4 border-0 text-xl focus:shadow-none">
    <CheckboxInput checked={task.completed} on:change={toggleCompleted} />
    <h2 class="mr-auto text-slate-700">
      {task.title}
    </h2>
    {#if task.description}
      <Icon
        src={ChevronDown}
        size="20"
        class="stroke-slate-800 transition {open ? 'rotate-180' : ''}"
      />
    {/if}
    <Menu class="relative">
      <MenuButton class="rounded-full border-0 p-2 hover:bg-gray-200">
        <Icon src={EllipsisVertical} size="20" class="stroke-slate-500" />
      </MenuButton>
  ...
  >
    {#if open && task.description}
      <DisclosurePanel
        class="mx-3 mt-2 rounded-lg border border-slate-200 bg-slate-50 p-4 text-slate-600"
